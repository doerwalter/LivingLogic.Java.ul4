...
{
	public Upper(SourcePart origin, Node obj)
	{
		super(origin, obj);
	}
	...
	protected SQLSnippet sqlOracle()
	{
		...
		switch (snippet.type)
		{
			case STR:
			case CLOB:
				return new SQLSnippet(snippet.type, "upper(", snippet, ")");
			default:
				complain(snippet);
		}
		return null;
	...
	{
		throw error("vsql.Upper({}) not supported!", snippet.type);
	}
	...
	public static class Function extends Unary.Function
	{
		...
		{
			return new Upper((SourcePart)args.get(1), (Node)args.get(0));
		}
	}
}
