...
public class IfExpression extends AST
{
	...
	public IfExpression(Location location, int start, int end, AST objIf, AST objCond, AST objElse)
	{
	...
	{
		if (objCond instanceof Const)
		{
			Object cond = ((Const)objCond).value;

			if (!(cond instanceof Undefined))
				return FunctionBool.call(cond) ? objIf : objElse;
		}
		return new IfExpression(location, start, end, objIf, objCond, objElse);
	}
