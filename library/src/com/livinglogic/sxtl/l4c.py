...
class CallMeth(AST):
	...
	def compile(self, registers, location):
		if len(self.args) == 0:
			...
			(r0, opcodes0) = self.args[0].compile(registers, location)
			(r1, opcodes1) = self.args[1].compile(registers, location)
			(r2, opcodes2) = self.args[2].compile(registers, location)
			opcodes += opcodes1
			opcodes += opcodes2
			opcodes += opcodes3
...
class ExprParser(spark.GenericParser):
	...
	expr_getslice12.spark = ['expr9 ::= expr9 [ expr0 : expr0 ]']
	...
	expr_getslice1.spark = ['expr9 ::= expr9 [ expr0 : ]']
	...
	expr_getslice2.spark = ['expr9 ::= expr9 [ : expr0 ]']
...
class StmtParser(ExprParser):
	...
	def __init__(self, scanner, start="stmt"):
		ExprParser.__init__(self, scanner, start)
	__init__.spark = ['for ::= ( name , name , ) in expr0']
